...
%
% State vector: z = [xw; xw_dot; theta; th_dot; x; x_dot]
...
c_x_x   = P_mat(2,1);  % Coeff for x_ddot from x
c_x_th  = P_mat(2,2);  % Coeff for x_ddot from theta
c_th_x  = P_mat(1,1);  % Coeff for th_ddot from x
c_th_th = P_mat(1,2);  % Coeff for th_ddot from theta

% G_vec = [b_th; b_x]
G_vec = inv_M_lin * Fu_vec;
b_x   = G_vec(2);      % Coeff for x_ddot from u
b_th  = G_vec(1);      % Coeff for th_ddot from u

% Build A and B matrices
% z_dot = A*z + B*u
% z = [xw; xw_dot; theta; th_dot; x; x_dot]
% z_dot = [xw_dot; xw_ddot; theta_dot; th_ddot; x_dot; x_ddot]
A = [0 1 0 0 0 0;            % xw_dot = xw_dot
     0 0 0 0 0 0;            % xw_ddot = u
     0 0 0 1 0 0;            % theta_dot = theta_dot
     0 0 c_th_th 0 c_th_x 0; % th_ddot = c_th_th*theta + c_th_x*x + b_th*u
     0 0 0 0 0 1;            % x_dot = x_dot
     0 0 c_x_th 0 c_x_x 0];   % x_ddot = c_x_th*theta + c_x_x*x + b_x*u
...
fprintf('Newly computed 6x6 A matrix (with d = %.2f):\n', d); disp(A);
fprintf('Newly computed 6x1 B matrix (with d = %.2f):\n', d); disp(B);

%% ========================================================================
% 3. LQR CONTROLLER DESIGN
% =========================================================================
fprintf('Designing LQR controller...\n');
% Penalize states: [xw, xw_dot, theta, th_dot, x, x_dot]
Q = diag([10, 10, 90, 50, 1000, 90]); % Penalties for all 6 states
R_lqr = 0.001; % Penalize control effort

% Check controllability
ctrb_rank = rank(ctrb(A, B));
fprintf('Controllability matrix rank: %d\n', ctrb_rank);
if ctrb_rank < size(A, 1)
    error('System is NOT controllable. (Rank is %d, expected %d)', ctrb_rank, size(A, 1));
else
    fprintf('System is controllable.\n');
end

[K, S, E] = lqr(A, B, Q, R_lqr);
fprintf('New LQR Gain K (1x6):\n'); disp(K);
...
% Plot 3: Wheel Position (xw)
subplot(3,1,3);
plot(t, z(:,1), 'LineWidth', 1.6, 'Color', [0.8 0.2 0.2]);
xlabel('Time [s]');
ylabel('Wheel pos xw [m]');
title('Wheel Position (Nonlinear Simulation)');
grid on;
...
function dz = robot_ball_dynamics(t, z, K, ...
    ...
    % The controller state is [xw, xw_dot, theta, th_dot, x, x_dot]
    % The state vector z is already in this order.
    state_for_lqr = z;
    
    % Control input u is defined as the wheel acceleration
    xw_ddot = -K * state_for_lqr; % This is u (1x6) * (6x1)
