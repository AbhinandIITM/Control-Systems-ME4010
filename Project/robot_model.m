...
% --- THIS IS THE FIX ---
% 'd' is the distance from wheel center to robot CoM (M_r)
% Based on your image and EOMs, this must be non-zero.
d = 0.15; % Using a reasonable value (from your other script)
% ---------------------

% Moments of Inertia
I_w = 0.5 * M_w * R^2;        % Inertia of one wheel
I_r = 1;                   % Inertia of robot body (given value)
I_b = 0.4 * m_b * r^2;      % Inertia of a solid sphere ball

% Derived parameters
l = h + r; % Height of robot CoM from ball center
...
Fg11 = g*m_b;         % d(f1)/dx
Fg12 = M_r*d*g + g*m_b*l; % d(f1)/dtheta  <-- This term now correctly uses d
Fg21 = 0;             % d(f2)/dx
...
% We reorder [x; theta] to match our state z
C_mat = inv_M_lin * Fg_mat * [0 0 1 0; 1 0 0 0]; % [x;theta] from [x;x_dot;th;th_dot]
G_vec = inv_M_lin * Fu_vec;

% Extract coefficients
% x_ddot = C_mat(2,1)*x + C_mat(2,2)*x_dot + C_mat(2,3)*theta + C_mat(2,4)*theta_dot
% th_ddot = C_mat(1,1)*x + C_mat(1,2)*x_dot + C_mat(1,3)*theta + C_mat(1,4)*theta_dot
c_x_x = C_mat(2,1);
c_x_th = C_mat(2,3);
c_th_x = C_mat(1,1);
c_th_th = C_mat(1,3);
...
Q = diag([10, 1, 10, 1]); % Penalize x and theta positions
